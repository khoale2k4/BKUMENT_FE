  ...
  async (
    ...
    try {
      ...
      return rejectWithValue({
        error: error.response?.data?.message || "common.errors.sendFailed",
        tempId,
      });
    }
  },
  ...
  async (
    {
      ...
      userIds,
      name,
    }: { type?: "GROUP" | "DIRECT"; userIds: string[]; name?: string },
    { rejectWithValue },
  ) => {
    try {
      const response = await chatService.createChat(type, userIds, name);
      console.log("Created chat response:", response);
      return response;
    } catch (error: any) {
      return rejectWithValue(
        error.response?.data?.message || "common.errors.createFailed",
      );
    }
  },
);
...
export const uploadConversationAvatarAsync = createAsyncThunk(
  "chat/uploadAvatar",
  async (
    { conversationId, file }: { conversationId: string; file: File },
    ...
    try {
      ...
      await updateConversationAvatar(
        conversationId,
        API_ENDPOINTS.RESOURCE.LINK_IMAGE_FILEID(assetId),
      );
...
export const uploadConversationNameAsync = createAsyncThunk(
  ...
  async (
    ...
    try {
      await chatService.updateConversationName(conversationId, name);
...
export const sendImageMessageAsync = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      console.log("Image uploaded, sending message with URL:", imageUrl);
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    addMessage: (state, action: PayloadAction<ChatMessage>) => {
      ...
      const isActiveChat =
        state.activeConversationId === newMessage.conversationId;

      if (isActiveChat) {
        const isDuplicate = state.currentMessages.some(
      ...
      .addCase(fetchMessagesByConversationId.fulfilled, (state, action) => {
        ...
        if (state.activeConversationId === action.payload.conversationId) {
          ...
          state.hasMoreMessages =
            action.payload.messages.length === action.payload.size;
        }
      })
    ...
    builder
      ...
      .addCase(sendMessageAsync.fulfilled, (state, action) => {
        ...
        if (state.activeConversationId === realMessage.conversationId) {
          const tempIndex = state.currentMessages.findIndex(
            (m) => m.tempId === tempId || m.id === tempId,
          );

          const socketIndex = state.currentMessages.findIndex(
          ...
          if (socketIndex !== -1) {
            ...
            state.currentMessages[tempIndex] = {
              ...realMessage,
              status: "sent",
              isSelf: true,
            };
      ...
      .addCase(sendMessageAsync.rejected, (state, action) => {
        const tempId =
          (action.payload as any)?.tempId || action.meta.arg.tempId;
