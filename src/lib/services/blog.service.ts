...
export const getPresignedUrl = async (
  ...
  const response = await httpClient.get(
    API_ENDPOINTS.RESOURCE.GET_PRESIGNED_URL(fileName),
  );
  return {
...
export const getMyBlogs = async (page: number, size: number): Promise<any> => {
  const response = await httpClient.get(
    API_ENDPOINTS.BLOGS.MY_BLOGS(page, size),
  );
...
export const getUserBlogs = async (
  ...
  const response = await httpClient.get(
    API_ENDPOINTS.BLOGS.USER_BLOGS(userId, page, size),
  );
  console.log("API Response for getUserBlogs WITH USER ID:", userId, response.data.result); // Debug log
  return response.data.result;
};
