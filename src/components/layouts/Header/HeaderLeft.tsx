...
export default function HeaderLeft() {
  ...
  return (
    // [SỬA]: Thêm shrink-0 để khối logo không bị méo. Giảm gap trên mobile.
    <div className="flex items-center gap-1 sm:gap-4 shrink-0">
      <button
        onClick={() => dispatch(toggleSidebar())}
        className={clsx(
          // [SỬA]: Giảm padding của nút menu trên mobile để nhường chỗ
          "p-1.5 sm:p-2 rounded-full transition-all duration-300 ease-in-out",
          isSidebarOpen
            ? "opacity-0 -translate-x-4 pointer-events-none"
            : "opacity-100 translate-x-0 cursor-pointer hover:bg-gray-100",
        )}
