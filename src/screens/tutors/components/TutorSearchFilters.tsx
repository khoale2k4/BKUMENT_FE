...
import { SearchFilters, Subject } from '@/lib/redux/features/tutorFindingSlice';
...
const TutorSearchFilters: React.FC<TutorSearchFiltersProps> = ({ 
  ...
  const availableTopics = useMemo(() => {
    ...
    const foundSubject = subjects.find(s => 
      s.name.toLowerCase() === filters.subjectName?.toLowerCase()
    );
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <input 
            list="topic-list"
            type="text" 
            placeholder={filters.subjectName ? "Chủ đề (VD: Giao tiếp)" : "Chọn môn học trước"} 
