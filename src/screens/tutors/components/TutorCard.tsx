...
const TutorCard: React.FC<TutorCardProps> = ({ data }) => {
  ...
  const handleAcceptApplication = () => {
    if (window.confirm(t('tutorCard.confirmApprove', 'Bạn có chắc chắn muốn DUYỆT hồ sơ của gia sư {{name}}?', { name: tutor.name }))) {
      ...
      */}
      <div className="relative flex flex-col md:flex-row gap-6 p-6 bg-white border border-gray-200 rounded-3xl shadow-sm hover:shadow-md transition-shadow duration-300">
        ...
        <div className="flex flex-col md:flex-row gap-6 flex-1">
          ...
          {/* Cột Thông Tin */}
          <div className="flex-1 mt-2 md:mt-0 pr-12 md:pr-0"> {/* Thêm pr-12 trên mobile để không đè vào nút Heart */}
            {isAdminView ? (
              ...
              {tutor.status === 'PENDING' ? (
                <>
                  <button onClick={handleAcceptApplication} className="w-full flex justify-center items-center gap-2 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-bold rounded-xl transition-all active:scale-95 shadow-sm">
                    <CheckCircle size={18} /> {t('tutorCard.approveBtn', 'Duyệt hồ sơ')}
                  </button>
                ...
                <button onClick={() => alert(t('tutorCard.viewUserDetailAlert', 'Xem chi tiết user: {{id}}', { id: tutor.id }))} className="w-full py-2.5 bg-slate-900 hover:bg-black text-white text-sm font-bold rounded-xl transition-all active:scale-95 shadow-sm">
