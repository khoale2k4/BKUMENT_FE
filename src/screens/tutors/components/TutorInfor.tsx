...
export const AdminTutorInfo: React.FC<{ tutor: TutorData['tutor'] }> = ({ tutor }) => (
...
export const UserTutorInfo: React.FC<{ tutor: TutorData['tutor'], matchingClasses: TutorData['matchingClasses'], totalMatchingClasses: number }> = ({ tutor, matchingClasses, totalMatchingClasses }) => {
  const subject = matchingClasses && matchingClasses.length > 0 ? matchingClasses[0].subjectName : "Đa môn học";
    ...
    <>
      ...
      <div className="flex flex-wrap items-center gap-4 mb-4 text-sm text-gray-600">
        <div className="flex items-center gap-1.5">
          ...
          <span><strong>{tutor.averageRating || "0.0"}</strong> Rating</span>
        </div>
        
        <div className="flex items-center gap-1.5">
          <Users size={16} className="text-blue-600" />
          <span><strong>{}</strong> Students</span>
        </div>
        
        <div className="flex items-center gap-1.5">
          <PlayCircle size={16} className="text-rose-500 fill-rose-100" />
          <span><strong>{totalMatchingClasses}</strong> Courses</span>
        </div>
      </div>
