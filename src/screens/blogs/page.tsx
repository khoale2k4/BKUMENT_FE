...
export default function BlogDetailPage(params: PageProps) {
  ...
  // 2. Lấy dữ liệu thực tế từ Redux Store (dữ liệu bạn vừa nhập bên trang Create)
  const { title, contentHTML, coverImage, visibility } = useAppSelector(
    (state) => state.blogs
  );

  // 3. Tạo BLOG_DATA_NEW mapping từ Redux
  const BLOG_DATA_NEW = {
    title: title || "Tiêu đề nháp (Chưa nhập)", // Fallback nếu chưa nhập
    coverImage: coverImage,
    contentHTML: contentHTML || "<p>Chưa có nội dung...</p>",
    visibility: visibility,
    type: "POST",
  };

  console.log("new blog at BlogDetailPage", BLOG_DATA_NEW);

  // 4. Quyết định hiển thị data nào.
  // Nếu có tiêu đề trong Redux (tức là đang xem trước bài viết vừa tạo) thì dùng BLOG_DATA_NEW
  // Ngược lại dùng BLOG_DATA cũ làm mẫu.
  const data = BLOG_DATA_NEW;
    ...
    <div className="min-h-screen bg-white font-sans pb-20">
      ...
      <article className="max-w-3xl mx-auto px-4 mt-8 sm:mt-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          {/* Ngày tháng tạm thời lấy hiện tại vì trong Redux chưa lưu ngày tạo */}
        ...
        {/* Profile section - Hardcode tạm thời hoặc lấy từ User Slice nếu có */}
