      ...
      {data.recommendationReason && (
        <div className="flex items-center gap-2 mb-4 px-2.5 py-1 bg-gray-50 rounded-lg w-fit border border-gray-100">
          ...
          <span className="text-[11px] text-gray-500 font-medium tracking-tight leading-none">
            {t(`common.recommendation.${data.recommendationReason.type}`, {
              title:
                data.recommendationReason.title ||
                t("common.recommendation.genericItem"),
            })}
          </span>
        </div>
      )}
      ...
      <div className="flex flex-col-reverse sm:flex-row justify-between gap-5 mb-5">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-gray-500 text-[15px] line-clamp-3 leading-relaxed font-normal">
            {data?.content?.startsWith("common.")
              ? t(data.content)
              : data.content ||
                data.description ||
                t("home.contentCard.noDescription")}
          </p>
        </div>

        {(data?.coverImage || data?.previewImageUrl) && (
          <div className="w-full sm:w-40 h-44 sm:h-28 flex-shrink-0 overflow-hidden rounded-xl bg-gray-50 border border-gray-100 shadow-sm transition-transform duration-500 group-hover:scale-[1.02]">
            <AuthenticatedImage
              src={data.coverImage || data.previewImageUrl || ""}
