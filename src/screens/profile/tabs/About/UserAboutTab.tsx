  ...
  if (isLoading) {
    return (
      <div className="flex justify-center py-32">
        <Loader2 className="animate-spin text-[#1a8917]" size={32} />
      </div>
    );
  }
        ...
        {isEditing ? (
          <div className="flex gap-3">
            <button
              ...
              className="px-5 py-2 text-sm font-semibold text-gray-500 hover:bg-gray-100 transition-colors rounded-full"
            >
            ...
            <button
              onClick={handleSave}
              disabled={isUpdating}
              className="flex items-center gap-2 px-6 py-2 bg-[#1a8917] hover:bg-[#156d12] text-white text-sm font-bold rounded-full transition-all shadow-sm disabled:opacity-70 active:scale-95"
            >
              {isUpdating ? (
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 bg-gray-50 text-gray-700 hover:bg-gray-100 font-semibold text-sm rounded-full transition-colors border border-gray-200"
          >
            <Edit3 size={16} />{" "}
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-10 mb-12 bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
        ...
        <div className="relative group shrink-0 flex flex-col items-center">
          <div
            ...
            className={`w-36 h-36 md:w-44 md:h-44 rounded-full overflow-hidden border-4 border-white shadow-lg bg-gray-50 relative ${
              isEditing
                ? "cursor-pointer hover:border-[#1a8917] transition-colors"
                : ""
            }`}
          >
        ...
        <div className="flex-grow w-full text-center md:text-left">
          ...
          {!isEditing && (
            <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-6">
              <div
                onClick={() =>
                  user?.id && router.push(`/profile/${user.id}/followers`)
                }
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">
                    {t("profile.user.about.followers", "Followers")}
                  </span>
              ...
              <div
                onClick={() =>
                  user?.id && router.push(`/profile/${user.id}/followings`)
                }
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">
                    {t("profile.user.about.following", "Following")}
                  </span>
                  <span className="font-bold text-gray-900 leading-none">
              ...
              <div className="flex items-center gap-2.5 px-4 py-2 bg-orange-50/50 border border-orange-100 rounded-2xl">
                ...
                <div className="flex flex-col items-start">
                  <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">
                    {t("profile.user.about.points", "Points")}
                  </span>
                  <span className="font-bold text-gray-900 leading-none">
                    {user?.points || 0}
                  </span>
          ...
          <div className="col-span-1 md:col-span-2">
            {!isEditing ? (
              <ProfileField
                ...
                value={
                  user?.university ||
                  t("profile.user.about.notUpdated", "Chưa cập nhật")
                }
                isEditing={false}
                onChange={() => {}}
