    ...
    <div className="max-w-4xl mx-auto animate-in fade-in duration-700 font-sans pb-10">
      ...
      <div className="flex justify-end mb-6 h-10 items-center">
        {isEditing ? (
          ...
          <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 px-5 py-2 bg-gray-50 text-gray-700 hover:bg-gray-100 font-semibold text-sm rounded-full transition-colors border border-gray-200">
            <Edit3 size={16} /> {t('profile.user.about.editProfile', 'Edit Profile')}
          </button>
        )}
      </div>
            ...
            {!isEditing ? (
              <>
                ...
                <p className="text-gray-500 text-base md:text-lg leading-relaxed max-w-2xl">
                  {user?.bio || t('profile.user.about.noBio', 'Chưa có lời giới thiệu nào.')}
                </p>
              </>
            ) : (
              <>
                <ProfileField label={t('profile.user.about.firstName', 'First Name')} name="firstName" value={formData.firstName} isEditing={true} onChange={handleInputChange} />
                <ProfileField label={t('profile.user.about.lastName', 'Last Name')} name="lastName" value={formData.lastName} isEditing={true} onChange={handleInputChange} />
                <div className="col-span-2 mt-2">
                  <ProfileField label={t('profile.user.about.bio', 'Bio')} name="bio" value={formData.bio} isEditing={true} onChange={handleInputChange} type="textarea" />
                </div>
              </>
            )}
          </div>
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-6">
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">{t('profile.user.about.followers', 'Followers')}</span>
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">{t('profile.user.about.following', 'Following')}</span>
              ...
              <div className="flex items-center gap-2.5 px-4 py-2 bg-orange-50/50 border border-orange-100 rounded-2xl">
                ...
                <div className="flex flex-col items-start">
                  <span className="text-xs text-gray-500 font-medium uppercase tracking-wider">{t('profile.user.about.points', 'Points')}</span>
      ...
      <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
          ...
          <ProfileField label={t('profile.user.about.email', 'Email Address')} name="email" value={user?.email} isEditing={false} onChange={() => { }} icon={<Mail size={16} className="text-gray-400" />} />
          <ProfileField label={t('profile.user.about.phone', 'Phone Number')} name="phone" value={formData.phone} isEditing={isEditing} onChange={handleInputChange} icon={<Phone size={16} className="text-gray-400" />} />
          <ProfileField label={t('profile.user.about.dob', 'Date of Birth')} name="dob" type="date" value={formData.dob} isEditing={isEditing} onChange={handleInputChange} icon={<Calendar size={16} className="text-gray-400" />} />
          <ProfileField label={t('profile.user.about.address', 'Address')} name="address" value={formData.address} isEditing={isEditing} onChange={handleInputChange} icon={<MapPin size={16} className="text-gray-400" />} />
          ...
          <div className="col-span-1 md:col-span-2">
            {!isEditing ? (
              ...
              <ProfileField
                label={t('profile.user.about.universityId', 'University ID')}
                name="universityId"
                type="number"
