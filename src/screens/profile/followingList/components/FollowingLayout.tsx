...
const FollowingLayout: React.FC<FollowingLayoutProps> = ({
  profileId,
  listType,
}) => {
  const { t } = useTranslation();
  ...
  // [Mobile UI:] Trên mobile, dùng state để điều khiển xem panel nào đang hiển thị
  const [mobileView, setMobileView] = useState<"list" | "detail">("list");

  const { viewedProfile, isViewedProfileLoading } = useAppSelector(
    (state) => state.profile,
  );
  const { token } = useAppSelector((state) => state.auth);
  const { viewedItems: items, viewedStatus: blogStatus } = useAppSelector(
    (state) => state.myBlogs,
  );
  ...
  // [Mobile UI:] Khi chọn user trên mobile thì chuyển sang view 'detail'
  ...
  return (
    // [Mobile UI:] Mobile = flex-col (stack dọc), Desktop = flex-row (2 cột)
    <div className="flex flex-col md:flex-row h-[calc(100vh-80px)] min-h-[500px] w-full bg-[#f0f2f5] font-sans">
      {/*
        [Mobile UI:] CỘT TRÁI (Sidebar - Danh sách)
        - Mobile: full width, ẩn khi đang xem detail (mobileView === 'detail')
        - Desktop: w cố định, luôn hiển thị
      */}
      <div
        className={`
          w-full md:w-[320px] lg:w-[360px] xl:w-[400px]
          shrink-0 h-full shadow-sm z-10 bg-white
      ...
      {/*
        [Mobile UI:] CỘT PHẢI (Chi tiết Profile)
        - Mobile: full width, chỉ hiện khi mobileView === 'detail'
        - Desktop: flex-1, luôn hiển thị
      */}
      <div
        className={`
          flex-1 h-full overflow-y-auto bg-[#f0f2f5] relative
          ${mobileView === "list" ? "hidden md:block" : "block"}
        `}
      >
        {/* [Mobile UI:] Nút Back trên mobile để quay lại danh sách */}
        {mobileView === "detail" && selectedUser && (
          <button
            onClick={() => setMobileView("list")}
            className="md:hidden flex items-center gap-2 text-blue-600 font-semibold text-sm px-4 py-3 bg-white w-full border-b border-gray-100 sticky top-0 z-20"
          >
            <svg
              className="w-4 h-4"
              fill="none"
        ...
        {/* State 1: Chưa chọn ai - chỉ hiện trên desktop */}
        {!selectedUser && (
          <div className="hidden md:flex items-center justify-center h-full text-gray-500">
            <span className="bg-white px-6 py-3 rounded-full shadow-sm border border-gray-200 text-sm">
              {t("profile.user.tabs.selectUserHint")}
            </span>
          </div>
        )}
        ...
        {/* State 3: API trả về thành công */}
        {selectedUser && !isViewedProfileLoading && viewedProfile && (
          // [Mobile UI:] pb thêm để tránh content bị che bởi bottom nav nếu có
          <div className="w-full animate-in fade-in duration-300 pb-10">
            <ProfileHeader profile={viewedProfile} />
            <ProfileAboutBox profile={viewedProfile} />
            ...
            <div className="mt-4 md:mt-6 mx-3 md:mx-auto md:max-w-4xl bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6">
              <h3 className="text-base md:text-xl font-bold text-gray-900 mb-4 md:mb-6 border-b border-gray-100 pb-3 md:pb-4">
                {t("people.profile.blogsBy", {
                  name: viewedProfile.fullName || viewedProfile.firstName,
                })}
              </h3>
              ...
              {blogStatus === "loading" ? (
                ...
                <div className="text-center py-8 md:py-10 text-gray-500 bg-gray-50 rounded-xl text-sm">
                  {t("people.profile.noBlogs")}
                </div>
              )}
            </div>
