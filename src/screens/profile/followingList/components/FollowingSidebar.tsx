...
const FollowingSidebar: React.FC<FollowingSidebarProps> = ({
  ...
  const currentLoading =
    listType === "followings" ? isFollowingLoading : isFollowersLoading;

  const titleText =
    listType === "followings" ? "Đang theo dõi" : "Người theo dõi";
  const emptyText =
    listType === "followings" ? "Chưa theo dõi ai." : "Chưa có người theo dõi.";
  const countText =
    listType === "followings" ? "người đang theo dõi" : "người theo dõi";
  ...
  useEffect(() => {
    if (currentData?.data && currentData.data.length > 0 && !selectedUserId) {
      onSelectUser(currentData.data[0]);
      console.log("Auto-selecting user:", currentData.data[0].id);
    }
  }, [currentData, selectedUserId, onSelectUser]);
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-200">
      ...
      <div className="p-3 sm:p-4 border-b border-gray-100">
        ...
        <div className="relative">
          ...
          {/* [Mobile UI:] text-sm cho input trên mobile */}
          <input
            type="text"
            placeholder="Tìm kiếm..."
            className="w-full pl-9 pr-4 py-2 bg-gray-100 border-transparent rounded-full text-sm focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-1.5 sm:p-2">
        {currentLoading && !currentData ? (
          ...
          <div className="space-y-0.5 sm:space-y-1">
            {/* [Mobile UI:] label nhỏ hơn */}
            <p className="px-2 pt-2 pb-1 text-xs sm:text-sm font-semibold text-gray-500">
              {currentData?.totalElements} {countText}
            </p>
            {users.map((user) => (
              <FollowingSidebarItem
        ...
        {/* Phân trang */}
        {currentData && currentData.totalPages > 1 && (
          // [Mobile UI:] padding nhỏ hơn, text nhỏ hơn
          <div className="flex justify-between items-center px-3 sm:px-4 py-3 sm:py-4 mt-2 border-t border-gray-100">
            <button
              onClick={() => setPage((p) => Math.max(1, p - 1))}
              disabled={page === 1}
              className="text-xs sm:text-sm font-semibold text-blue-600 disabled:text-gray-300 active:opacity-70"
            >
              Trang trước
            </button>
